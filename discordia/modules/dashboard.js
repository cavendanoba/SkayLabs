...
function buildKPICards(ingresos, deudas, stockBajo, rotacionInventario) {
  ...
  const varHTML = ingresos.variacion !== null
    ? `<span class="text-xs font-normal px-2 py-0.5 rounded-full mt-2 bg-white/20 text-white inline-block">
    ...
    ${cards.map(c=>`
      <article class="${c.bg} border ${c.bc} rounded-2xl p-4 md:p-5 shadow-sm hover:shadow-md transition">
        <div class="flex items-start justify-between">
          ...
          <span class="text-[10px] uppercase tracking-[0.12em] ${c.lc} font-medium text-right leading-tight max-w-[90px]">${c.label}</span>
        </div>
...
function buildDeudas(deudas) {
  if (!deudas.length) return `
    <article class="bg-emerald-50 border border-emerald-100 rounded-2xl p-6 flex items-center gap-4 shadow-sm">
      ...
      <div>
        <p class="text-emerald-800 text-lg" style="font-family:'Playfair Display',serif;font-weight:600;">Sin deudas pendientes</p>
  ...
  return `
    <article class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
      <div class="bg-gradient-to-r from-[#6d165a] to-[#a0346e] px-5 py-4 flex items-center justify-between">
        <h3 class="text-white text-lg" style="font-family:'Playfair Display',serif;font-weight:600;">⚠️ Deudas activas</h3>
        <span class="text-xs bg-white/20 text-white px-3 py-1 rounded-full font-normal">Total ${formatCurrency(total)}</span>
      </div>
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-[#fdf2f7]">
            <tr>
              ...
              <th class="p-3 text-center text-xs uppercase tracking-widest text-[#6d165a] font-medium">Teléfono</th>
              <th class="p-3 text-right text-xs uppercase tracking-widest text-[#6d165a] font-medium">Debe</th>
            </tr>
          </thead>
          <tbody>
            ${deudas.map(d=>`
              <tr class="border-b border-gray-100 hover:bg-[#fdf7fa] transition">
                <td class="p-3">
                  <p class="text-gray-900 text-sm">${d.customer_name || 'Sin nombre'}</p>
                </td>
  ...
  return `
    <article class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
      <div class="bg-gradient-to-r from-[#ecd9ff] to-[#ffd5e3] px-5 py-4">
        <h3 class="text-white text-lg" style="font-family:'Playfair Display',serif;font-weight:600;">🏆 Top productos</h3>
          ...
          <li class="py-3 ${i<productos.length-1?'border-b border-gray-100':''}">
            <div class="flex items-center justify-between mb-2">
              <div class="flex items-center gap-2 min-w-0">
                ...
                <p class="text-sm text-gray-900 truncate">${p.product_name}</p>
              </div>
  ...
  return `
    <article class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
      <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <h3 class="text-[#6d165a] text-lg" style="font-family:'Playfair Display',serif;font-weight:600;">🕐 Ventas recientes</h3>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead class="bg-[#fdf2f7]">
            <tr>
              <th class="p-3 text-left text-xs uppercase tracking-widest text-[#6d165a] font-medium">Cliente</th>
              <th class="p-3 text-center text-xs uppercase tracking-widest text-[#6d165a] font-medium">Canal</th>
              <th class="p-3 text-center text-xs uppercase tracking-widest text-[#6d165a] font-medium">Estado</th>
              <th class="p-3 text-right text-xs uppercase tracking-widest text-[#6d165a] font-medium">Total</th>
            </tr>
          </thead>
          <tbody>
            ${ventas.map(v=>{
              ...
              return `
                <tr class="border-b border-gray-100 hover:bg-[#fdf7fa] transition">
                  <td class="p-3"><p class="text-gray-900 text-sm">${v.customer_name||'Sin nombre'}</p><p class="text-xs text-gray-400">${fecha}</p></td>
